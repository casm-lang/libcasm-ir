...
TEST( libcasm_ir__type_enum, make_and_get )
{
    ...
    auto v = libstdhl::Memory::make< EnumerationType >( example );
    ASSERT_TRUE( v != nullptr );

    EXPECT_STREQ( v->name().c_str(), "example" );
    EXPECT_STREQ( v->description().c_str(), "example" );

    auto w = libstdhl::Memory::make< EnumerationType >( example );
    ASSERT_TRUE( w != nullptr );

    EXPECT_TRUE( v != w );
    EXPECT_TRUE( *w == *w );

    auto a = libstdhl::Memory::get< EnumerationType >( example );
    auto b = libstdhl::Memory::get< EnumerationType >( example );
