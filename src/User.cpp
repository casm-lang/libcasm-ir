...
User::User( const char* name, Type* type, Value::ID id )
: Value( name, type, id )
, m_uses()
{
}
...
{
    libstdhl::Log::info( "removeUse: %s in %s", user.label(), this->label() );
...
void User::replaceAllUsesWith( Value& value )
{
    for( auto u : uses() )
    {
        libstdhl::Log::info( "%s -> %s", u->def().label(), u->use().label() );
