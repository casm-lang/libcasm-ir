...
{
    assert(
        0 && "invalid to check 'classof' Constant< V >, use ConstantValue" );
    return false;
}
...
{
    static BooleanConstant* cache[ 2 ] = { new BooleanConstant( false, true ),
...
: Constant< Type::Bit >(
      ".bit", type, this->value, defined, Value::BIT_CONSTANT )
{
    this->value[ 0 ] = value;
}
