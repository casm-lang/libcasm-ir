...
std::string Feature::name( void ) const
{
    return m_name;
}

std::size_t Feature::hash( void ) const
{
    return libstdhl::Hash::combine( classid(), std::hash< std::string >()( name() ) );
}

u1 Feature::operator==( const Value& rhs ) const
{
    if( this == &rhs )
    {
        return true;
    ...
    const auto& other = static_cast< const Feature& >( rhs );
...
u1 Feature::classof( Value const* obj )
{
