...
libcasm_ir::Specification* libcasm_ir::AstToCasmIRPass::getSpecification(
    void ) const
{
...
void libcasm_ir::AstToCasmIRPass::visit_init( InitNode* node )
{
    ...
    RulePointerConstant* ir_init
        = RulePointerConstant::create( node->identifier.c_str() );
...
void libcasm_ir::AstToCasmIRPass::visit_function_def(
    FunctionDefNode* node, const std::vector< std::pair< T, T > >& inits )
{
    VISIT;
    ...
    Function* ir_function = new Function( node->sym->name.c_str(), ftype );
    assert( ir_function );
...
void libcasm_ir::AstToCasmIRPass::visit_derived_def_pre( FunctionDefNode* node )
{
    ...
    for( i32 i = 0; i < node->sym->arguments_.size(); i++ )
    {
        ...
        ir_derived->addParameter(
            Identifier::create( param_types[ i ], param_ident
                //, ir_derived
                ) );
    }
...
void libcasm_ir::AstToCasmIRPass::visit_derived_def(
    FunctionDefNode* node, T expr )
{
    VISIT;
    string x;
    for( auto& a : node->sym->arguments_ )
    ...
    assert(
        Value::isa< Derived >( current_scope.back() ) and "invalid scope!" );
...
void libcasm_ir::AstToCasmIRPass::visit_update(
    UpdateNode* node, T func, T expr )
{
    VISIT;
    // printf( "%p -> %p\n", node, node->func );
    // printf( "%p -> %p\n", node, node->expr_ );
...
void libcasm_ir::AstToCasmIRPass::visit_update_dumps(
    UpdateNode* node, T func, T expr )
{
    VISIT;
    FIXME;
...
void libcasm_ir::AstToCasmIRPass::visit_update_subrange(
    UpdateNode* node, T func, T expr )
{
    VISIT;
    FIXME;
...
void libcasm_ir::AstToCasmIRPass::visit_call(
    CallNode* node, std::vector< T >& args )
{
    VISIT;
...
void libcasm_ir::AstToCasmIRPass::visit_ifthenelse(
    IfThenElseNode* node, T cond )
{
    VISIT;
    ...
    assert( ir_cond );
    assert( ( Value::isa< Instruction >( ir_cond )
                and ir_cond->getType()->getIDKind() == Type::BOOLEAN )
...
void libcasm_ir::AstToCasmIRPass::visit_case(
    CaseNode* node, T val, const std::vector< T >& case_labels )
{
    ...
    Value* ir_expr = lookup< Value >( node->expr );
    assert( ir_expr );
    assert( Value::isa< Instruction >( ir_expr )
...
T libcasm_ir::AstToCasmIRPass::visit_expression(
    BinaryExpression* node, T lhs, T rhs )
{
    VISIT;
...
T libcasm_ir::AstToCasmIRPass::visit_expression_single(
    UnaryExpression* node, T val )
{
    VISIT;
    // printf( "%s, %p\n", operator_to_str( node->op ).c_str(), node->left_ );
...
T libcasm_ir::AstToCasmIRPass::visit_function_atom(
...
T libcasm_ir::AstToCasmIRPass::visit_function_atom_subrange(
...
void libcasm_ir::AstToCasmIRPass::visit_derived_function_atom_pre(
    FunctionAtom* node, std::vector< T >& args )
{
    VISIT;
}

T libcasm_ir::AstToCasmIRPass::visit_derived_function_atom(
    FunctionAtom* node, T expr )
{
    VISIT;
    string x;
    // printf( "derived: %s, %s\n"
...
T libcasm_ir::AstToCasmIRPass::visit_undef_atom( UndefAtom* node )
{
    ...
    {
        case TypeType::RULEREF:
            ir_const = RulePointerConstant::create();
            break;
        case TypeType::BOOLEAN:
            ir_const = BooleanConstant::create();
            break;
        case TypeType::BIT:
            ir_const = BitConstant::create( node->type_.bitsize );
            break;
        case TypeType::INTEGER:
            ir_const = IntegerConstant::create();
            break;
        case TypeType::STRING:
            ir_const = StringConstant::create();
            break;
        default:
...
T libcasm_ir::AstToCasmIRPass::visit_bit_atom( IntegerAtom* node )
{
    ...
    BitConstant* ir_const
        = BitConstant::create( (u64)node->val_, node->type_.bitsize );
    assert( ir_const );
    ast2casmir[ node ] = ir_const;
...
T libcasm_ir::AstToCasmIRPass::visit_list_atom(
    ListAtom* node, std::vector< T >& args )
{
    VISIT;
    FIXME;
    return 0;
...
T libcasm_ir::AstToCasmIRPass::visit_number_range_atom(
    NumberRangeAtom* node, T start, T end )
{
    VISIT;
    FIXME;
    return 0;
...
T libcasm_ir::AstToCasmIRPass::visit_builtin_atom(
    BuiltinAtom* node, std::vector< T >& args )
{
    VISIT;
