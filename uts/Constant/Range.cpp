...
TEST( libcasm_ir__constant_range, of_integer_constants )
{
    const auto i = libstdhl::get< IntegerType >();
    const auto t = libstdhl::make< RangeType >( i );

    const auto m = libstdhl::get< IntegerConstant >( -4 );
    const auto n = libstdhl::get< IntegerConstant >( 13 );

    const auto v = libstdhl::get< RangeConstant >( t, *m, *n );
